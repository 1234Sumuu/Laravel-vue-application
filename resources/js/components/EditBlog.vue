    ...
    <div class="card">
        <div class="card-body">
            <div class="d-flex justify-content-between pb-2 mb-2">
                <h5 class="card-title">Update Blog Data</h5>
                <div>
                    <router-link :to="{ name: 'blogs' }" class="btn btn-success"
                        >Go Back</router-link
                    >
                </div>
            </div>
...
export default {
    ...
    created() {
        this.$axios.get("/sanctum/csrf-cookie").then((response) => {
            this.$axios
                .get(`/api/blogs/edit/${this.$route.params.id}`)
                .then((response) => {
    ...
    methods: {
        ...
        updateBlog(e) {
            this.$axios.get("/sanctum/csrf-cookie").then((response) => {
                ...
                this.$axios
                    .blog(
                        `/api/blogs/update/${this.$route.params.id}`,
                        formData,
                        config
                    )
                    .then((response) => {
